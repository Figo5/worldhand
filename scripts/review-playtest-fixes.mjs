...
// 1. Uncapped Seed accumulation in the REAL UI: a crafted valid save (seeds 24,
//    a two-pair hand = nominal 16) shows `Gains 16 Seeds` in the preview, and
//    after the play the HUD reads 40 (24 + 16, no cap) and the chronicle
//    carries the identical summary (preview == commit == log).
...
// ---------------- Fix 2: uncapped Seed accumulation in preview → commit → log ------
...
if (!/Gains 16 Seeds/.test(pvText)) throw new Error('preview missing "Gains 16 Seeds"')
if (/overflow|Credited/.test(pvText)) throw new Error(`preview still shows the cap clause: ${pvText}`)
...
if (!/40/.test(seedsHud)) throw new Error(`HUD should read 40 (24 + 16, uncapped), got ${seedsHud}`)
...
if (!played || !/Gains 16 Seeds/.test(played)) throw new Error('chronicle missing "Gains 16 Seeds"')
if (/overflow|Credited/.test(played)) throw new Error(`chronicle still shows the cap clause: ${played}`)
...
if (!/Gains 16 Seeds/.test(pvNarrow)) throw new Error('"Gains 16 Seeds" missing at 480px')
if (/overflow|Credited/.test(pvNarrow)) throw new Error(`cap clause still present at 480px: ${pvNarrow}`)
...
console.log('PLAYTEST-FIX CHECKS PASSED (uncapped Seeds, final-epoch results, reload-inert)')