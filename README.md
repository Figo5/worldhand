...
- **Unlimited epochs, ONE cumulative-Growth target each**: 45 → 110 → 360, then a formula escalates indefinitely. **Early advance**: the moment cumulative Growth reaches the epoch target, the epoch closes immediately (unused plays and discards are forfeited, exactly as in Balatro). Every epoch opens the market → next-epoch loop; the run ends only when lives run out or Flourishing collapses. Reload cannot duplicate rewards or life deductions (rewards apply once inside the engine commit; a finished run is inert).
