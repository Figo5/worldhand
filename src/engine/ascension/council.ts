...
export function marketOffers(s: AscensionState, era: number, reroll: number, m: RunMods): Offer[] {
  ...
  for (let i = 0; i < m.marketSlots - 2; i++) take(cards, 'card', (c) => RARITY_PRICE[c.rarity])
  for (let i = 0; i < 2; i++) take(decrees, 'decree', (d) => d.price)
  if (LEGENDARY_SALE_AFTER.includes(era)) {
    ...
    if (left.length) offers.push({ kind: 'legendary', id: stream(s.seed, 'ascension', 'legend-sale', era).pick(left), price: LEGENDARY_PRICE, sold: false })
  }
  return offers
}
