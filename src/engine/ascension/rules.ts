...
export const OMENS: readonly { level: number; name: string; text: string }[] = [
  ...
  { level: 3, name: 'Pressing Crises', text: 'Every crisis has +10% pressure.' },
  { level: 4, name: 'Thin Council', text: 'The Council market has one fewer offer, and rerolls cost 1 more.' },
  { level: 5, name: 'Fragile World', text: 'The world begins with 2 Resolve instead of 3.' },
  { level: 6, name: 'Deep Strain', text: 'Strain and inequality in crises count 50% more.' },
  { level: 7, name: 'Short Ages', text: 'One fewer hand every era.' },
  { level: 8, name: 'The Long Night', text: 'The final crisis gains +10 pressure for every Resolve the world has lost.' },
]
export const MAX_OMEN = OMENS.length
export const omenActive = (omen: number, level: number) => omen >= level

export function startingResolve(omen: number): number { return omenActive(omen, 5) ? 2 : 3 }
...
export function runMods(s: AscensionState, era = s.era): RunMods {
  ...
  const m: RunMods = {
    handsBonus: omenActive(omen, 7) ? -1 : 0,
    ...
    crisis: {
      ...
      pressurePct: omenActive(omen, 3) ? 10 : 0,
