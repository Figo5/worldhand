...
## Reconciled non-match-penalty experiment (rejected)

The implementation worker and reviewer both terminated on provider failures before producing a completed independent review. The candidate was preserved before surgical reversion at `.hermes/experiments/non-match-penalty-d02b14c/` (full diff, metric, capture script, and stats). The hypothesis was: a `NON_MATCH_PENALTY=3` contrastive penalty would raise regional decision influence and make all-three-awake at least as influential as the starting build. Astra measured the candidate on 30 seeds / 360 plays: **25/360 = 6.9%** starting build and **7/360 = 1.9%** all three awake. The monotonicity prediction failed, so the penalty is not shipped and was not tuned further.

The retained `scripts/flip-rate.mjs` defines a flip as a different selected card subset between the specialization-on argmax and the all-specializations-nulled argmax on the same recorded play state. It does not by itself mean a different poker category, a strict preference reversal, or a meaningful score margin; ties and tie-breaking are included unless separately classified. Same seed alone is insufficient for causal comparison when selection choices change future trajectories. On the restored additive v4 scoring, the metric reports **14/360 = 3.9%** as-shipped and **5/360 = 1.4%** all-awake. This is low-impact evidence, not a claim of strategic success.

The solver expansion comment and policy were corrected independently of the rejected scoring experiment: specialized wakes are considered when affordable and their category is in/near the hand. On the unchanged additive v4 scoring and the same disjoint `eval-0..29` set, LOOK=30 remains **20/30 (67%)**, versus the previously reported 20/30 baseline; measured change **0 percentage points**. This is a bounded solver result, not a human win-rate estimate. The final default remains v4 with targets `[45,110,360]`, SAVE_VERSION=4, and the existing save-rejection path unchanged.
